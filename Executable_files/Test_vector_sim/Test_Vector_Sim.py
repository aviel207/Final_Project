...
class hardware_class:
    ...
    def input_data(self):
        ...
        with open("input_data.txt", "w") as file:
            # Write two hexadecimal values per line
            for i in range(0, len(data_to_send)):
                line = data_to_send[i:i+1].hex()
                file.write(line + '\n')
...
class software_class:
    ...
    def data_hash(self):
        ...
        C_d = C_d[-(d//4):] 
        print('SOFTWARE: ',C_d)
        with open("soft_hash.txt", "w") as file:
            # Write two hexadecimal values per line
            for i in range(0, len(C_d), 2):  # Increment by 2 to get pairs of characters
                line = C_d[i:i+2]
                file.write(line + '\n')        #with open("soft_hash.txt", "w") as output_file:
        # Write the string to the file
            #output_file.write(C_d)
        return C_d
...
class md6_class:
    ...
    def test_vector(self):
        ...
        with open("vector_data.txt", "w") as output_file:
            # Iterate through the array
            for item in vector:
                # Convert each element to a string and write it to the file followed by a newline
